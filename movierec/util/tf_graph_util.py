...
NOTE: File copied from
https://raw.githubusercontent.com/tensorflow/tensorflow/v1.14.0/tensorflow/python/framework/graph_util_impl.py
The rest of the code might be running in Tensorflow v1.13.0-rc0 due to compatibility issues with TensorRT
 container. However, freezing the embedding layer in that version results in an error while loading the graph
 in TensorRT (Tensorflow C API). This file allows to freeze the graph with using the code from version v1.14.0.
"""
